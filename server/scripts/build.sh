...
BUILD_DIRNAME="build"
MAIN_BINARY_OUTPUT_FILENAME="example-api-server.bin"
...
echo "Building example API server code '${MAIN_GO_FILEPATH}'..."
if ! CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build -o "${MAIN_BINARY_OUTPUT_FILEPATH}" "${MAIN_GO_FILEPATH}"; then
  echo "Error: Code build of the example API server failed" >&2
  exit 1
fi
...
echo "Building example API server into a Docker image named '${image_name}'..."
if ! docker build -t "${image_name}" -f "${dockerfile_filepath}" "${server_root_dirpath}"; then
  echo "Error: Docker build of the example API server failed" >&2
  exit 1
fi
echo "Successfully built Docker image '${image_name}' containing the example API server"
